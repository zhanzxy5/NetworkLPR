%% Function wraper for the timing inference procedure
function [bpara_data, main_dir] = SigTimeInference(date, intersectionID, start_hour, end_hour)
...
initial_phase_length = [60, 60]; % in seconds
C = 0.1;
eta = 0.01;
max_phase_len = 90;
v_val = [0.5;1];
MMaxIter = 1000;
Ncycle = 1500; % maximum number of phases
...
b_prev_para = [initial_phase_length(dir), initial_phase_length(3-dir)];
...
%         I = min(I1,I2);
    ...
    %     [Abx, fval] = fmincon(@(Ab)Lobj(Ab, V, T, X, b_prev, eta), Ab0, [], [], [], [], lb, ub, @(Ab)Lcon(Ab, V, T, X, b_prev, eta), options);
        ...
        fprintf('Intersection=%s\tCycle=%d\tDir=%d:\tPhase %d - %d:\tSizeDir1=%d\tSizeDir2=%d\tPhase length=%f\tminX=%f\n', ...
            intersectionID, floor((Titer + 1)/2), dir, round(phase_start), round(phase_start + b), ...
...
%         hold on;
        ...
        b = b_prev;
        fprintf('Intersection=%s\tCycle=%d\tDir=%d:\tPhase %d - %d:\tPhase length=%f\n', intersectionID, floor((Titer + 1)/2), dir, round(phase_start), round(phase_start + b), b);
    end
